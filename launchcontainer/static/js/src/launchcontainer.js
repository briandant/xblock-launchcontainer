  ...
  $(document).ready(
    function () {
      ...
      $('#launcher_form').submit(function (event) {
        ...
        $launcher.find('iframe')[0].contentWindow.postMessage($(this).serialize(), "{{ API_url }}");
        return false;
      });
      ...
      window.addEventListener("message", function (event) {
        ...
        if(event.data.status === 'siteDeployed') {
          $launch_notification.html(event.data.html_content);
          ...
          if ($status_code == 400) {
            ...
            for (i=0; i<$errors.length; i++) { 
              var $msg = $errors[i][0] + ": " + $errors[i][1][0] + " "; 
            }
          } else if ($status_code == 403) {
            var $msg = "Your request failed because the token sent with "
                        +"your request is invalid. "
          } else if ($status_code == 404) {
            var $msg = "That project was not found. "; 
          } else if ($status_code == 503) {
            var $msg = event.data.errors + " ";
          }
          var $final_msg = "<p class='error'>An error occured in your request.</p>" 
                           + $msg 
                           + "Please contact "
